...
const generateAuthTokens = async (user) => {
  const expireaccessTokenIn = Math.floor(Date.now()/1000)+ config.jwt.accessExpirationMinutes *60;
  const tokenType= tokenTypes.ACCESS;
  const jwtToken = generateToken(user._id,expireaccessTokenIn,tokenType);
  return{
      access:{
      token:jwtToken,
      expires:new Date(expireaccessTokenIn*1000)
    }
  }
};
